...
const SearchUser = async (event) => {
  ...
  try {
    if (selectedUser.value) {
      userList.value = storeUser.users.filter(
        (c) => c.id === selectedUser.value
      );
    } else {
      userList.value = storeUser.users;
    }
  } catch (error) {
...
const openNewUserModal = async () => {
  await openModal(
    defineAsyncComponent(() => import("@/components/AddUserModal.vue")),
    {
      // item: item,
    }
  )
...
const editUser = async (item) => {
  await openModal(
    defineAsyncComponent(() => import("@/components/EditUserModal.vue")),
    {
      item: item,
      onUpdateSuccess: SearchUser, // Pass the function
    }
  )
    // runs when modal is closed via confirmModal
    .then((data) => {
...
const ffffff = async (item) => {
  ...
  if (result.isConfirmed) {
    try {
      const response = await UserAPI.delete(item.id);
      if (response.data.ok) {
        showSweetAlert({
          ...
          SearchUser(); // Refresh the user list
        });
      } else {
        showSweetAlert({
...
const deleteUser = async (item) => {
  showSweetAlert({
    ...
    if (alertResult.value.isConfirmed) {
      try {
        const { data } = await UserAPI.delete(item.id);
        if (data.ok) {
          showSweetAlert({
            title: "Deleted!",
...
<template>
  ...
  <div class="container-fluid">
    ...
    <Spinner v-if="storeUser.loading" />
          ...
          <div class="basic-form">
            <div class="row">
              <div class="mb-3 col-md-3">
                ...
                <v-select
                  :options="storeUser.users"
                  v-model="selectedUser"
                  placeholder="Choose User"
                  :reduce="(user) => user.id"
                  label="name"
                  class="form-control p-0"
                  :class="{ 'is-invalid': formSubmitted && !selectedUser }"
                />
              </div>
            </div>
    ...
    <div class="col-lg-12">
      <div class="card">
        <div class="card-body">
          <div class="basic-form">
            <div class="row">
            <div style="text-align: end; color:blueviolet">{{ userList.length}} rows</div>
